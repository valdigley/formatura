              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    step="0.1"
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                    placeholder="0 (+ taxa, - desconto)"
                  />
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    Positivo: taxa (ex: 3.5% cartão) | Negativo: desconto (ex: -10%)
                  </p>
                </div>
                ...
                <div>
                  ...
                  <select
                    value={discount >= 0 ? 'fee' : 'discount'}
                    onChange={(e) => {
                      const isDiscount = e.target.value === 'discount';
                      setDiscount(isDiscount ? -Math.abs(discount) : Math.abs(discount));
                    }}
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                  >
                    <option value="fee">Taxa (Cartão/Serviço)</option>
                    <option value="discount">Desconto (Promoção)</option>
                  </select>
                </div>
                    ...
                    {discount !== 0 && (
                      <div className={`text-xs ${
                        ...
                        {discount < 0 
                          ...
                          : `Taxa: R$ ${(finalAmount - originalAmount).toLocaleString('pt-BR')}`
                        }
                      </div>
                    )}
                  </div>
              ...
              {discount !== 0 && (
                <div className={`mt-3 p-3 border rounded-lg ${
                  ...
                  <p className={`text-sm ${
                    ...
                    {discount < 0 ? (
                      <>
                        <strong>Desconto concedido:</strong> {Math.abs(discount)}% sobre R$ {originalAmount.toLocaleString('pt-BR')} = 
                        <strong> R$ {(originalAmount - finalAmount).toLocaleString('pt-BR')} de desconto</strong>
                      </>
                    ) : (
                      <>
                        <strong>Taxa aplicada:</strong> {discount}% sobre R$ {originalAmount.toLocaleString('pt-BR')} = 
                        <strong> R$ {(finalAmount - originalAmount).toLocaleString('pt-BR')} de taxa</strong>
                      </>
                    )}
                  </p>
                </div>
